...
fn test_buffer_pool_lru_eviction() {
    ...
    for i in 0..10 {
        let page = buffer_pool.new_page(PageType::Leaf).expect("Failed to create page");
        let page_id = page.read().header.page_id;
        buffer_pool.unpin_page(page_id, false).expect("Failed to unpin");
        pages.push(page_id);
    }
    ...
    for i in 10..20 {
        ...
        assert!(result.is_ok(), "Page {} should be in buffer pool", i);
        buffer_pool.unpin_page(page_id, false).expect("Failed to unpin");
    }
...
fn test_buffer_pool_access_pattern_affects_eviction() {
    ...
    for _ in 0..10 {
        let page = buffer_pool.fetch_page(page_ids[0]).expect("Fetch failed");
        buffer_pool.unpin_page(page_ids[0], false).expect("Unpin failed");
    }
    ...
    assert!(result.is_ok(), "Frequently accessed page 0 should remain in pool");
    buffer_pool.unpin_page(page_ids[0], false).expect("Unpin failed");
...
fn test_pin_counter_prevents_eviction() {
    ...
    let pinned_page = buffer_pool.new_page(PageType::Leaf).expect("Failed to create page");
    ...
    assert_eq!(pinned_data.header.page_id, pinned_page_id, "Pinned page should remain in pool");

    // Now unpin it
    drop(pinned_data); // Release the read lock
    buffer_pool.unpin_page(pinned_page_id, false).expect("Failed to unpin");
...
fn test_multiple_pins_require_multiple_unpins() {
    ...
    buffer_pool.unpin_page(page_id, false).expect("Unpin 1 failed");
    buffer_pool.unpin_page(page_id, false).expect("Unpin 2 failed");
    buffer_pool.unpin_page(page_id, false).expect("Unpin 3 failed");
    buffer_pool.unpin_page(page_id, false).expect("Unpin 4 failed");
...
fn test_dirty_page_tracking_and_writeback() {
    ...
    for i in 0..5 {
        ...
        // Modify the page (add some data)
        {
            ...
            page_guard.add_cell(&key, &value).expect("Failed to add cell");
        }

        // Unpin and mark as dirty
        buffer_pool.unpin_page(page_id, true).expect("Failed to unpin");
        page_ids.push(page_id);
    }
    ...
    for page_id in &page_ids {
        ...
        let reloaded_page = buffer_pool.fetch_page(*page_id).expect("Failed to reload page");

        // Verify data is intact (data was persisted)
        let page_guard = reloaded_page.read();
        assert!(page_guard.header.cell_count > 0, "Reloaded page should have cells");
...
fn test_clean_pages_not_written_on_eviction() {
    ...
    for _ in 0..3 {
        let page = buffer_pool.new_page(PageType::Leaf).expect("Failed to create page");
        let page_id = page.read().header.page_id;
        buffer_pool.unpin_page(page_id, false).expect("Failed to unpin"); // false = not dirty
    }
...
fn test_dirty_flag_cleared_after_flush() {
    ...
    for _ in 0..10 {
        let p = buffer_pool.new_page(PageType::Leaf).expect("Failed to create page");
