use monodb_server::daemon::Server;
...
fn get_env_filter() -> EnvFilter {
    if std::env::var_os("RUST_LOG").is_some() {
        ...
        EnvFilter::new("monodb=debug,server_lib=debug,info")
    } else {
        EnvFilter::new("monodb=info,server_lib=info,warn")
    }
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let config = monodb_server::config::Config::load_from_path("config.toml")?;
