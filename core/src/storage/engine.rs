...
impl StorageEngine {
	...
	fn load_tables(&self) -> MonoResult<()> {
		...
		for (name, entry) in &catalog.tables {
			// Create table with schema from catalog to maintain column order
	...
	/// Read a row with schema-aware column ordering
	pub fn read(
		&self,
		table_name: &str,
		page_id: PageId,
		...
		// Read with schema context to maintain column order
	...
	/// Scan a table with schema-aware ordering
		...
		F: FnMut(PageId, u16, &[u8]) -> MonoResult<bool>,
	{
		...
		// Scan tuples with schema context for proper column ordering
